...
def create_round():
    ...
    winnerContract=requests.post("http://116.203.30.147:9053/script/p2sAddress", data=json.dumps({'source': winnerScript.replace('|', '').replace("\n", "\\n").replace("\t", "")}), headers=headers).json()
